...
export async function renderAI(container) {
    ...
    simForm.onsubmit = async (e) => {
        ...
        try {
            ...
            const balances = res.projected_balance_6m;
            const max = Math.max(...balances);

            container.innerHTML = balances.map((b, i) => `
                <div style="flex: 1; min-width: 10px; background: rgba(var(--color-2), 0.5); height: ${(b / max) * 100}%; position: relative; border-top-left-radius: 4px; border-top-right-radius: 4px;" title="Month ${i + 1}: $${b.toFixed(0)}">
                </div>
            `).join('');
